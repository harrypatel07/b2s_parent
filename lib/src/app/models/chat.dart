...
class ChatModel {
  ...
  final List<MessageBubble> listMessage;
  ChatModel(
      {this.peerId,
      this.avatarUrl,
      this.name,
  ...
  static List<ChatModel> dummyData = [
    ChatModel(
        ...
        listMessage: [
          MessageBubble(body: "Hôm nay trường có sự kiện abc", fromMe: false),
          MessageBubble(body: "Sự kiện diễn ra khi nào vậy cô?", fromMe: true),
        ]),
    ChatModel(
        peerId: 2,
        ...
        message: "hmmmm....",
        listMessage: [
          MessageBubble(body: "hmmmm....", fromMe: false),
        ]),
    ChatModel(
        peerId: 3,
...
class Chatting {
  ...
  Chatting.fromMessages(Messages messsages) {
    peerId = messsages.senderId;
    timestamp = int.parse(messsages.timestamp);
    var dateTime = DateFormat('dd MMM kk:mm')
        .format(DateTime.fromMillisecondsSinceEpoch(timestamp));
    datetime = dateTime;
    name = messsages.receiverName;
    message = messsages.content;
    listMessage = [];
  }
}
...
class Contacts {
  ...
  dynamic avatarUrl;
  String name;
  Contacts({this.peerId, this.avatarUrl, this.name});
  Contacts.fromDocumentSnapShot(ResPartner resPartner) {
